...
`quint` is preinstalled on PATH — invoke it directly (no `npx`). **Wrap `quint` invocations (and only `quint`) in `timeout 30`** so a pathological spec cannot hang the turn (e.g. `timeout 30 quint typecheck …`); a well-formed spec typechecks and simulates in well under a second, so 30s only ever fires on a genuine hang. Do **not** apply this timeout to project build/test commands (`go build`, `cargo build`, `npm test`, etc.) — those legitimately take minutes and a short timeout will kill them; run them without a timeout or a generous one.
