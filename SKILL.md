...
**Guidelines directory:** `${CLAUDE_SKILL_DIR}/guidelines/` — read the relevant guideline file before each major step.
...
```
Read ${CLAUDE_SKILL_DIR}/guidelines/language-constraints.md
Read ${CLAUDE_SKILL_DIR}/guidelines/quint-syntax.md
```

Then read the appropriate template:
- For standard systems (state machines, smart contracts, algorithms):
  ```
  Read ${CLAUDE_SKILL_DIR}/guidelines/spec-template.md
  ```
- For distributed/consensus protocols (BFT, message-passing, multi-node):
  ```
  Read ${CLAUDE_SKILL_DIR}/guidelines/choreo-template.md
  ```
...
|---|---|
| Consensus / BFT / distributed | `${CLAUDE_SKILL_DIR}/guidelines/examples/consensus.qnt` + `two_phase_commit.qnt` |
| Distributed with Choreo framework | `${CLAUDE_SKILL_DIR}/guidelines/examples/two_phase_commit_choreo.qnt` |
| DeFi / tokens / smart contracts | `${CLAUDE_SKILL_DIR}/guidelines/examples/coin.qnt` + `erc20.qnt` |
| Finance / blockchain / multi-asset | `${CLAUDE_SKILL_DIR}/guidelines/examples/bank.qnt` |
| State machines / games / turn-based | `${CLAUDE_SKILL_DIR}/guidelines/examples/tictactoe.qnt` |
| Simple / puzzles / logic | `${CLAUDE_SKILL_DIR}/guidelines/examples/prisoners.qnt` |
| General / unsure | `${CLAUDE_SKILL_DIR}/guidelines/examples/coin.qnt` (best all-around pattern demo) |

If you need to look up a specific Quint operator's signature, read `${CLAUDE_SKILL_DIR}/guidelines/builtins.md`.
...
```
Read ${CLAUDE_SKILL_DIR}/guidelines/error-handling.md
```

Determine the quint command: try `quint` first. If it fails with "command not found", fall back to `npx @informalsystems/quint` for all subsequent commands.
...
```
Read ${CLAUDE_SKILL_DIR}/guidelines/verification.md
```
...
```bash
quint run {witness_file} --main={witness_module} --invariant={witness_name} --max-steps=100 --max-samples=100
```
...
```bash
quint run {file} --main={module} --invariant={invariant_name} --max-steps=200 --max-samples=500
```
...
- **quint not installed**: If `quint` fails with "command not found", fall back to `npx @informalsystems/quint` for all commands. Suggest the user install globally with `npm i -g @informalsystems/quint` for faster runs.
